...
gameToNode = collections.defaultdict(list)
...
class Node:
    ...
    def uct(self, child):
        # print(child.mean(), Node.c * math.sqrt(math.log(self.numSimulations) / child.numSimulations))
        return child.maxValue + Node.c * math.sqrt(math.log(self.numSimulations) / child.numSimulations)
    
    def traverse(self):
        node = self
        while node.isExpanded() and not node.state.isTerminalState():
            node = max(node.children, key=self.uct)
        if node.state.isTerminalState():
            # print('term')
            return node
        else:
